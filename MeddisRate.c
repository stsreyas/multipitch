...
double updateCochleaWithoutTransduction(int chan_index, float sigval, int tim)
{
	double zz, bm;
...
channel *getChannels(void)
{
	...
	ALLOCATE_ARRAY(chan,NUM_CHANNEL);
	for (i=0;i<NUM_CHANNEL;i++)
		memcpy((void*)&(chan[i]),(void*)&(cochlea[i]),sizeof(channel));
